...
public class ContactPage {
	...
	public Composite createContent(CTabFolder shell) {
		init();
		...
		final Composite leftComposite = new Composite(composite, SWT.BORDER
				| SWT.V_SCROLL);
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			@Override
			public void widgetSelected(SelectionEvent e) {
				PopupList list = new PopupList(display.getShells()[0]);
				list.setItems(OPTIONS);
				Rectangle rectangle = new Rectangle(
						leftComposite.getBounds().x + 15, leftComposite
								.getBounds().y, 150, 50);
